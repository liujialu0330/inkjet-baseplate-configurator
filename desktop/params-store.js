// 路径解析 + 读写 inkjet-baseplate-params.json (主进程数据访问层)
// - 开发模式(从项目 npm start 跑): 直接读写技能根目录的 inkjet-baseplate-params.json, 即 generator.py 读取的
...
function paramsPath(){
  if (!app.isPackaged) {
    // desktop/ 的上一级 = 技能根; 那里的 inkjet-baseplate-params.json 与 generator.py 同源
    return path.join(__dirname, '..', 'inkjet-baseplate-params.json');
  }
  const up = path.join(app.getPath('userData'), 'inkjet-baseplate-params.json');
